...
pygame.time.set_timer(enemy_spawn_timer, 3000)
...
def main():
    ...
    while running:
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.MOUSEBUTTONDOWN: # mouse click
                ...
                if click_state and selected_card:
                    ...
                    if grid_x <= 18 and grid_y <= 24:
                        ...
                        if game.player_elixir.can_subtract_elixir(new_card):
                            ...
                            # card placement
                            print(f'new_card.x: {new_card.x // TILE_SIZE}, new_card.y: {new_card.y // TILE_SIZE}')
                ...
                if game.enemy_elixir.elixir_count > 0:
                    ...
                    temp_enemy = selected(x=0, y=0, is_friendly=False)
                    if game.enemy_elixir.can_subtract_elixir(temp_enemy):
                        game.enemy_elixir.subtract_elixir(temp_enemy)
                        cards.enemy_placed.append(temp_enemy)
                        print(f'Enemy spawned: {temp_enemy}')
        ...
        # card attacking
        all_cards = cards.placed_card + cards.enemy_placed
        for card in all_cards:
            ...
            if not attacking:
                ...
                card.x, card.y = card.move(card.x, card.y, 9, 4)   
                    #if card.x >= 8:
